...
my $d = 5;   # distance to exon boundary outside which potential splicing sites are marked
...
my $rseqex;
my %pnebq;     # position near exon boundary 
...
for (my $i = 0; $i < @es; $i++) {
    ...
    foreach my $p (($es[$i]-$d)..($es[$i]-1)) {
	$pnebq{$p} = 1;
    }
    foreach my $p (($ee[$i]+1)..($ee[$i]+$d)) {
	$pnebq{$p} = 1;
    }
}
